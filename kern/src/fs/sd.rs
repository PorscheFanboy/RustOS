...
#[no_mangle]
fn wait_micros(micro_seconds: u64) {
    spin_sleep(Duration::from_micros(micro_seconds));
}
...
impl Sd {
    ...
    pub unsafe fn new() -> Result<Sd, io::Error> {
        ...
        return Err(io::Error::new(io::ErrorKind::TimedOut, "not found"));
    }
}
...
impl BlockDevice for Sd {
    ...
    fn read_sector(&mut self, n: u64, buf: &mut [u8]) -> io::Result<usize> {
        let k = unsafe { sd_readsector(n as i32, &mut buf[0] as *mut u8) };
        if k <= 0 {
            panic!("failed to read sd card");
